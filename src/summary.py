...
def make_conf_summary(writer, conf, steps, labels, prefix='', pca=None, logvar=None,
                      palette_type=None):
    if conf.shape[1] > 1:
        if pca is None:
        ...
        if palette_type is not None:
            ...
            sns.scatterplot(
                x=pc[:, 0], y=pc[:, 1], hue=rainbow_labels, palette=palette, alpha=.01
            )
            plt.xlabel('PC1')
            plt.ylabel('PC2')
            plt.legend([], [], frameon=False)
            writer.add_figure(prefix + "Conformation - Rainbow PCA", f, global_step=steps)
        ...
        if labels is not None:
            ...
            fig, _ = analysis.plot_by_cluster(pc[:, 0], pc[:, 1], n_classes, labels, centers_ind=centers_i,
                                              annotate=True, dpi=96, figsize=(5, 5), s=5)
            writer.add_figure(prefix + "Conformation - Colored PCA", fig, global_step=steps)
    ...
    if conf.shape[1] == 1:
        if labels is not None:
            ...
            for i in range(i_min, i_max + 1):
                ...
                plt.hist(conf[indices_at_i].flatten(), bins=50, histtype='step', label=str(i))
            plt.legend(loc='best')
            plt.grid(True)
            writer.add_figure(prefix + "Conformation - Colored Histogram", fig, global_step=steps)
        else:
            fig = plt.figure(dpi=96, figsize=(6, 4))
            plt.plot(conf[:, 0], 'k.')
            writer.add_figure(prefix + "Conformation - Histogram", fig, global_step=steps)
    ...
    if logvar is not None:
        ...
        plt.xlabel('Std. Dev.')
        plt.legend(loc='best')
        writer.add_figure(prefix + 'Conformation - Histogram of Std. Dev.', fig, global_step=steps)
...
def single_loss_summary(writer, per_image_loss, angles, steps, suffix):
    ...
    if angles is not None:
        ...
        writer.add_figure('Out-of-Plane Error vs. Loss' + suffix, fig, global_step=steps)

        fig = plt.figure(dpi=96, figsize=(7, 5))
        plt.hist(angles[losses > x_90], bins=100, histtype='step')
        plt.xlabel('Out-of-Plane Error (deg)')
        writer.add_figure('Out-of-Plane Errors 10% Junk' + suffix, fig, global_step=steps)
...
def make_pose_summary(writer, rots, trans, rotmat_gt, trans_gt, steps, shift=False, prefix='',
    ...
    fig = plt.figure(dpi=96, figsize=(10, 4))
    plt.subplot(111)
    plt.hist2d(azimuth.flatten(), elevation.flatten(), cmap='Reds', bins=100,
    ...
    if rotmat_gt is not None:
        if not view_direction_only:
            ...
            rots, rot_pred_aligned, rot_gt_aligned = metrics.align_view_dir(rotmat_gt, rots)

        angles, mean_oop_error, median_oop_error = metrics.get_angular_error(
            rotmat_gt, rot_pred_aligned)
        writer.add_scalar(prefix + 'Out-of-Plane Angle - Mean Error (deg.)', mean_oop_error, steps)
        writer.add_scalar(prefix + 'Out-of-Plane Angle - Median Error (deg.)', median_oop_error, steps)

        fig = plt.figure(dpi=96, figsize=(7, 5))
        plt.hist(angles.flatten(), bins=100, histtype='step')
        plt.xlabel('Error (deg.)')
        writer.add_figure(prefix + 'Out-of-Plane Angle - Histogram of Errors', fig, global_step=steps)
        ...
        plt.subplot(131)
        plt.hexbin(euler_gt[:, 0], euler_pred_aligned[:, 0], cmap='Oranges', gridsize=35)
        plt.xlabel('alpha (gt)')
        ...
        plt.subplot(133)
        plt.hexbin(euler_gt[:, 2], euler_pred_aligned[:, 2], cmap='Purples', gridsize=35)
        plt.xlabel('gamma (gt)')
        plt.ylabel('gamma (pred)')
        writer.add_figure(prefix + 'Euler Angles - Prediction vs. Ground Truth', fig, global_step=steps)

        fig = plt.figure(dpi=96, figsize=(15, 5))
        plt.subplot(131)
        plt.hexbin(euler_pred[:, 0], euler_gt_aligned[:, 0], cmap='Oranges', gridsize=35)
        ...
        plt.ylabel('beta (gt)')
        plt.subplot(133)
        plt.hexbin(euler_pred[:, 2], euler_gt_aligned[:, 2], cmap='Purples', gridsize=35)
        plt.xlabel('gamma (pred)')
        plt.ylabel('gamma (gt)')
        writer.add_figure(prefix + 'Euler Angles - Ground Truth vs. Prediction', fig, global_step=steps)
        ...
        writer.add_scalar(prefix + 'In-Plane Angle - Mean Error (deg.)', mean_ip_error, steps)
        writer.add_scalar(prefix + 'In-Plane Angle - Median Error (deg.)', median_ip_error, steps)

        fig = plt.figure(dpi=96, figsize=(7, 5))
        plt.hist(ip_angular_errors.flatten(), bins=100, histtype='step')
        plt.xlabel('Error (deg.)')
        writer.add_figure(prefix + 'In-Plane Angle - Histogram of Errors', fig, global_step=steps)

        if shift:
            trans = torch.tensor(trans).float()
            trans_pred_corr, trans_gt_corr, trans_metrics, dist = metrics.get_trans_metrics(
                trans_gt, trans, rots, correct_global_trans=True)
            writer.add_scalar(prefix + 'Translation - RMSE (pix.)', trans_metrics['rmse'], steps)
            writer.add_scalar(prefix + 'Translation - RMedSE (pix.)', trans_metrics['rmedse'], steps)
            writer.add_scalar(prefix + 'Translation - Mean Absolute Error (pix.)', trans_metrics['mae'], steps)
            writer.add_scalar(prefix + 'Translation - Median Absolute Error (pix.)', trans_metrics['medae'], steps)

            fig = plt.figure(dpi=96, figsize=(7, 5))
            plt.hist(np.sqrt(dist).flatten(), bins=100, histtype='step')
            plt.xlabel('Error (pix.)')
            writer.add_figure(prefix + 'Translation - Histogram of Errors', fig, global_step=steps)

            fig = plt.figure(dpi=96, figsize=(10, 4))
            plt.subplot(121)
            plt.hexbin(trans_gt_corr[:, 0], trans_pred_corr[:, 0], cmap='Blues', gridsize=35, extent=(-5, 5, -5, 5))
            plt.xlabel('x gt (pix.)')
            plt.ylabel('x pred (pix.)')
            plt.subplot(122)
            plt.hexbin(trans_gt_corr[:, 1], trans_pred_corr[:, 1], cmap='Blues', gridsize=35, extent=(-5, 5, -5, 5))
            plt.xlabel('y gt (pix.)')
            plt.ylabel('y pred (pix.)')
            writer.add_figure(prefix + 'Translation - Prediction vs. Ground Truth', fig, global_step=steps)
