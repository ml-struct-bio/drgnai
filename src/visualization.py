...
def interactive_filtering(workdir: str, epoch: int, kmeans: int,
    ...
    if os.path.exists(train_configs_file):
        ...
        if (not os.path.exists(path_configs_file)
    ...
    conf_fls = [fl for fl in os.listdir(os.path.join(workdir, 'out'))
                if 'conf.' in fl]
    if not conf_fls:
        raise NotImplementedError("Filtering is not available for the output "
    ...
    rot, trans = utils.load_pkl(os.path.join(
    ...
    if kmeans == -1:
        ...
        if not os.path.exists(kmeans_dir):
            raise ValueError("This experiment does not contain results for "
    ...
    save_option = input(
        "Do you want to save the selection? (yes/no): ").strip().lower()

    if save_option == 'yes':
        filename = input("Enter filename to save selection (absolute, "
...
class SelectFromScatter:
    ...
    def __init__(self,
                 data_table: pd.DataFrame, pre_indices: list[int]) -> None:
        ...
        self.select_cols = [
            col for col in data_table.select_dtypes('number').columns
            if col != 'index'
            ]
        ...
        self.menu_col = RadioButtons(cax, labels=['None'] + self.select_cols,
                                     active=0)
    ...
    def plot(self) -> None:
        ...
        self.pik_txt = self.main_ax.text(
            0.99, 0.01, f"# of selected particles: {len(self.indices)}",
            size=11, fontstyle='italic', ha='right', va='bottom',
    ...
    def choose_points(self, verts: np.array) -> None:
        self.indices = np.where(PlotPath(verts).contains_points(
            self.scatter.get_offsets()))[0]
    ...
    def hover_points(self, event: Event):
        ...
        if event.inaxes == self.main_ax:
            ...
            if cont:
                ...
                self.annot.xy = pos
                ant_lbls = [str(int(self.data_table.iloc[x]['index']))
                            for x in ix["ind"]]
